...
*/
  ...
  // check the signature
  $secret = '8aa0673c378b28f149aa46885f995d32'; // insert your secret
  ...
  $sms_user = new User($user = $cuid, $world = 2);
    ...
    if($test){
      ...
      if(preg_match("/failed/i", $_GET['status'])) {
        ...
        if ($sms_user->data()->referer != 0) {
          ...
          $ref_user_data_query = DB::getInstance(2)->query("SELECT `flc` FROM `users_data` WHERE `id` = ".$sms_user->data()->referer." ");
          ...
          DB::getInstance(2)->update('users_data', $sms_user->data()->referer, array('flc' => $ref_user_data->flc + $ref_flc));
          DB::getInstance(2)->insert('credit_history', $ref_fields);
        }
      }
    }
  } else {
  ...
  DB::getInstance(2)->insert('credit_history', $sms_fields);
